...
- Always run Python scripts using: `conda run -n IITB python script.py`
- Never use bare `python3` or `python` directly
- If writing a temporary script to a file first, run it as:
  `conda run -n IITB python <raw path to temp_script.py>`